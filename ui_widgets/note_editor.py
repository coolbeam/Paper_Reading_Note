...
from qdarktheme.qtpy.QtGui import QAction, QActionGroup, QFont, QIcon, QTextOption, QPainter, QColor, QTextFormat, QTextCursor, QDropEvent, QPalette, QDesktopServices, QKeySequence
from qdarktheme.qtpy.QtWidgets import (
    QApplication, QColorDialog, QFileDialog, QVBoxLayout, QHBoxLayout, QLabel, QMainWindow, QMenuBar,
    QMessageBox, QSizePolicy, QStackedWidget, QStatusBar, QToolBar, QToolButton, QSplitter, QLineEdit, QPushButton,
...
from utils.markdown_style import GITHUB_MARKDOWN_STYLE, GITHUB_MARKDOWN_STRYLE_DARK
import datetime
import webbrowser
...
class Editor(QPlainTextEdit):
    ...
    def load_paper_sample(self, paper_sample: Paper_Sample):
        self.paper_sample = paper_sample
        # print('载入了吗？',self.paper_sample.to_dict)
    ...
    def insertFromMimeData(self, source) -> None:
        ...
        if source.hasFormat('application/x-qt-image'):
            ...
            if_save = a.save(save_path, 'jpg', quality)
            if if_save:
                self.insertPlainText('![image](./%s)' % im_name)
                # print('粘贴图片:', if_save, save_path)
                # print(self.paper_sample.to_dict)
                self.paper_sample.image_ls.append(im_name)  # 没有用？不能保存进去？
                # print(self.paper_sample.image_ls)
            else:
                print('保存图片失败')
    ...
    def keyPressEvent(self, e):
        # 处理tab和shift tab事件
        if e.key() == Qt.Key.Key_Tab or e.key() == Qt.Key.Key_Backtab:
            handle_func = None
            ...
            return
        # 处理ctrl+b加粗字体事件
        if e.key() == Qt.Key.Key_B and QApplication.keyboardModifiers() == Qt.ControlModifier:
            print('按下了ctrl+B')
        # 处理ctrl+D加换行符<br>事件 todo 如果选中了文本怎么办
        if e.key() == Qt.Key.Key_D and QApplication.keyboardModifiers() == Qt.ControlModifier:
            print('按下了ctrl+D')
            cursor = self.textCursor()
            assert isinstance(cursor, QTextCursor)
            cursor.insertText("<br>\n")
            # cursor.setPosition(cursor.position() + 4)#无法移动光标
        # 处理ctrl+Q加换行符颜色事件 todo 如果选中了文本怎么办, 想把光流放在中间
        if e.key() == Qt.Key.Key_Q and QApplication.keyboardModifiers() == Qt.ControlModifier:
            print('按下了ctrl+Q')
            cursor = self.textCursor()
            assert isinstance(cursor, QTextCursor)
            cursor.insertText("<font color=#A00000>  </font>")
...
class Paper_Note_Editor_preview(QWidget):
    def __init__(self, settings: Settings, paper_sample: Paper_Sample, *args):
        ...
        # 使用markdown2
        md2 = Markdown(extras=["cuddled-lists",  # "fenced-code-blocks",
                               "footnotes", 'header-ids', 'numbering', 'pyshell', 'strike', 'tables',  # "code-friendly",'spoiler',
                               'wiki-tables', 'task_list', 'mdx-math'])  # 'tag-friendly',
        self.md2 = md2.convert
        # 使用markdown, #markdown2Mathjax#failed to install on win10, 所以不能用'pymdownx.arithmatex'
        self.md = markdown.Markdown()# 参考https://facelessuser.github.io/pymdown-extensions/
        ...
        self.year_editor = QLineEdit()  # 年份
        self.produ_editor = QLineEdit()  # 出处
        self.btn_tag_edit = QPushButton()  # 标签设置
        ...
        # ===== init preview
        self._is_loading = False  # 是否正在load数据,如果是因为load数据导致的text修改,就不能改变其修改日期
    ...
    def setup_actions(self):
        ...
        self.btn_save.setToolTip('保存设置和数据, 快捷键ctrl+s')
    ...
    def setup_ui(self):
        ...
        # ===== 论文信息输入部分： 标题，年份，出处，阅读程度，code链接，pdf下载链接，还有添加标签的模块(这个感觉有点难，暂时不弄)
        title_container = make_h_layout([QLabel("标题"), self.title_editor])  # === 标题 一行
        self.btn_tag_edit.setText('标签设置')
        ...
        pro_container = make_h_layout([QLabel("出处"), self.produ_editor, ], margins=[10, 0, 0, 0])  # 左侧margin
        read_container = make_h_layout([QLabel("阅读进度"), self.reading_progress, ], margins=[10, 0, 0, 0])
        btn_container = make_h_layout([self.btn_save, self.btn_tag_edit, ], margins=[10, 0, 0, 0])
        year_pro_read_hbox_container = make_h_layout([year_container, pro_container, read_container, btn_container])
    ...
    def load_paper_sample(self, paper_sample: Paper_Sample):
        ...
        self.paper_sample = paper_sample
        self.editor.load_paper_sample(paper_sample)
        self._is_loading = True  # 是否正在load数据,如果是因为load数据导致的text修改,就不能改变其修改日期
        # 载入paper笔记内容
        ...
        self._is_loading = False  # 载入完毕,这时候如果再修改内容,就会改变修改日期
        self.update_preview()
    ...
    def show_web_preview(self, md_text):
        if self.conf.editor_markdown == 'markdown2':
            html_md = self.md2(md_text)  #
        elif self.conf.editor_markdown == 'commonmark':
            html_md = commonmark.commonmark(md_text)
        elif self.conf.editor_markdown == 'markdown':
            print('使用md')
            html_md = self.md.convert(md_text)  #
        else:
            raise ValueError('markdown的语法设置错误: %s' % self.conf.editor_markdown)
        math_jax_str = '''
'''
        if self.conf.theme == 'dark':
            text = f"<head><style>{GITHUB_MARKDOWN_STRYLE_DARK}</style>{math_jax_str}</head><body>{html_md}</body>"
        elif self.conf.theme == 'light':
            text = f"<head><style>{GITHUB_MARKDOWN_STYLE}</style>{math_jax_str}</head><body>{html_md}</body>"
        else:
