...
class Note_list(QWidget):
    ...
    def setup_ui_actions(self):
        self.paper_tree_view.setColumnCount(3)  # 一共三列
        self.paper_tree_view.setHeaderLabels(["年份", '出处', '标题'])
    ...
    def setup_ui_layout(self):
        ...
        for p_key in self.paper_data.keys():
            ...
            child.setToolTip(2, paper_sample.title)
        ...
        # self.paper_tree_view.setCurrentItem(child_ls[-1])

        globale_vbox = QVBoxLayout(self)
        search_container = self._make_h_layout([self.bt_addnote, self.search_text, self.search_button, self.search_option])
    ...
    def init_set_item(self):
        self.paper_tree_view.setCurrentItem(self.tree_view_child_ls[list(self.tree_view_child_ls.keys())[-1]])
    ...
    def on_currentItemChanged(self, item, preitem):
        ...
        #         print(preitem.paper_sample.title)
